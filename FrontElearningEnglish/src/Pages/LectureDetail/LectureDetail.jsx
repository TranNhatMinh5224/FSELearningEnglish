...
export default function LectureDetail() {
    ...
    const fetchLectureDetail = useCallback(async (lectureIdToLoad, updateUrl = true) => {
        if (isFetchingLectureRef.current) return; // prevent re-entrancy
    ...
    useEffect(() => {
        ...
        // Only run this effect when the module/lesson/course changes.
        // lectureId is handled by a separate effect to avoid re-fetching the entire tree when the URL is updated.
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [moduleId, courseId, lessonId]);

    // Fetch lecture detail when lectureId changes (e.g., browser back/forward)
    // Using ref to track previous lectureId to prevent unnecessary re-fetches
    const prevLectureIdRef = useRef(lectureId);
    
    useEffect(() => {
        // Only react to lectureId changes from browser navigation (back/forward)
        // Skip if lectureId hasn't changed or if we're still loading the tree
        if (lectureId && 
            !loadingTree && 
            ...
            // Only fetch if it's a different lecture
            if (parsedId !== currentId) {
                setSelectedLectureId(parsedId);
                fetchLectureDetail(parsedId, false);
            }
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [lectureId]); // Only depend on lectureId to prevent infinite loop
