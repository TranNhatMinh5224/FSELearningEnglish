...
export default function AdminModuleLectureDetail() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [moduleRes, lecturesRes, courseRes, lessonRes] = await Promise.all([
        ...
        lectureService.getAdminLectureTree(moduleId),
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const res = await lectureService.deleteAdminLecture(lectureId);
      if (res.data?.success) {
  ...
  const handleReorder = async (reorderList) => {
    ...
    try {
      const res = await lectureService.reorderAdminLectures(reorderList);
      if (res.data?.success) {
        fetchData();
      } else {
      ...
      <Container fluid className="lesson-detail-content px-4">
        ...
        {loading ? (
          ...
          <div className="no-modules-message">
            ...
            <p>Module này hiện đang trống. Hãy bắt đầu xây dựng nội dung bài giảng để hoàn thiện chương trình học.</p>
            <Button 
              variant="primary" 
